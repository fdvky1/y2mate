...
func Get(url string) (result *Y2MateRes, err error){
	client := resty.New()
	res, err := client.R().
...
func GetDownloadUrl(vid string, key string) (result *Y2MateCVRes, err error){
	client := resty.New()
	res, err := client.R().
				SetFormData(map[string]string{
					"vid": vid,
...
func main(){
	result, err := Get("https://youtu.be/ioW9iGDpQyw")
	if err != nil {
		fmt.Println(err)
	}
	for _, v := range result.Links.Mp3 {
		url, err := GetDownloadUrl(result.Vid, v.K)
		if err != nil {
			fmt.Println(err)
		}	
		fmt.Println(url.Dlink)
	}
}
